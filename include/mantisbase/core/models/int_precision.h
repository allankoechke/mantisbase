...
namespace mb {
    ...
    MANTISBASE_API [[nodiscard]] IntPrecision defaultIntPrecision();

    /** @return `true` for `U8`..`U64` precisions. */
    MANTISBASE_API [[nodiscard]] bool isUnsignedIntPrecision(IntPrecision precision);

    /** @return String token such as `"i32"` or `"u64"`. */
    MANTISBASE_API [[nodiscard]] std::string intPrecisionToString(IntPrecision precision);

    /** Parse a precision token; throws on unknown input. */
    MANTISBASE_API [[nodiscard]] IntPrecision intPrecisionFromString(const std::string &token);

    /**
     * Parse precision from a field JSON object.
     * Accepts string tokens (i8, u32, …) or legacy numeric bit widths (8, 16, 32, 64) as signed.
     */
    MANTISBASE_API [[nodiscard]] IntPrecision intPrecisionFromField(const nlohmann::json &field);

    /** Map precision to the SOCI indicator type used for bind/read. */
    MANTISBASE_API [[nodiscard]] soci::db_type intPrecisionToSociType(IntPrecision precision);

    /** @return `{min, max}` inclusive range for signed precisions. */
    MANTISBASE_API [[nodiscard]] std::pair<int64_t, int64_t> intPrecisionSignedRange(IntPrecision precision);

    /** @return `{min, max}` inclusive range for unsigned precisions. */
    MANTISBASE_API [[nodiscard]] std::pair<uint64_t, uint64_t> intPrecisionUnsignedRange(IntPrecision precision);

    /** @return Error message if `token` is not a valid precision string, else `nullopt`. */
    MANTISBASE_API [[nodiscard]] std::optional<std::string> validateIntPrecisionToken(const std::string &token);

    /** @return Error message if `value` is out of range for `precision`, else `nullopt`. */
    MANTISBASE_API [[nodiscard]] std::optional<std::string> validateIntFieldValue(const nlohmann::json &value,
                                                                   IntPrecision precision);

    /** Validate `min`/`max` constraint keys on an int field definition. */
    MANTISBASE_API [[nodiscard]] std::optional<std::string> validateIntConstraintBounds(const nlohmann::json &field);

    /** Bind an int JSON value into SOCI `vals` using the correct width/type. */
    MANTISBASE_API void bindIntFieldValue(soci::values &vals, const std::string &field_name, const nlohmann::json &value,
                           IntPrecision precision);

    /** Read an int column from a SOCI row into JSON using the field precision. */
    MANTISBASE_API [[nodiscard]] nlohmann::json readIntFieldValue(const soci::row &row, size_t index, IntPrecision precision);
} // namespace mb
