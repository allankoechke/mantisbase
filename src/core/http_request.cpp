...
std::string mb::MantisRequest::getBody() const { return m_req.body; }

std::string MantisRequest::getRemoteAddr() const {
  // Check X-Forwarded-For header (validate to prevent spoofing)
  if (hasHeader("X-Forwarded-For")) {
    auto forwarded = getHeaderValue("X-Forwarded-For", "", 0);

    // Take first IP (client IP), validate format
  ...
  const auto direct_ip = m_req.remote_addr;
...
int MantisRequest::getRemotePort() const { return m_req.remote_port; }

std::string MantisRequest::getLocalAddr() const { return m_req.local_addr; }

int MantisRequest::getLocalPort() const { return m_req.local_port; }
...
std::string MantisRequest::getTrailerValue(const std::string &key,
                                           size_t id) const {
...
httplib::Match MantisRequest::matches() const { return m_req.matches; }
...
bool MantisRequest::hasPathParams() const { return !m_req.path_params.empty(); }
...
void MantisRequest::registerDuktapeMethods() {
  ...
  // `req.getHeader("Authorization", "", 0)` -> Return Authorization value or
  // default `req.getHeader("Authorization", "Default Value", 0)` -> Return
  // Authorization value or default `req.getHeader("Some Key", "Default Value",
  // 1)` -> Return 'Some Key' value if exists of index '1' or default
  dukglue_register_method(ctx, &MantisRequest::getHeaderValue, "getHeader");
  dukglue_register_method(ctx, &MantisRequest::getHeaderValueU64,
                          "getHeaderU64");
  // `req.getHeaderCount("key")` -> Count for header values given the header key
  dukglue_register_method(ctx, &MantisRequest::getHeaderValueCount,
                          "getHeaderCount");
  ...
  dukglue_register_method(ctx, &MantisRequest::getTrailerValueCount,
                          "getTrailerCount");
  ...
  dukglue_register_method(ctx, &MantisRequest::getQueryParamValueCount,
                          "getQueryParamCount");
  ...
  dukglue_register_method(ctx, &MantisRequest::getPathParamValueCount,
                          "getPathParamCount");

  // `req.isMultipartFormData()` -> Return true if request type is of
  // Multipart/FormData
  dukglue_register_method(ctx, &MantisRequest::isMultipartFormData,
                          "isMultipartFormData");
  ...
  // `req.remoteAddr`
  dukglue_register_property(ctx, &MantisRequest::getRemoteAddr, nullptr,
                            "remoteAddr");
  // `req.remotePort`
  dukglue_register_property(ctx, &MantisRequest::getRemotePort, nullptr,
                            "remotePort");
  // `req.localAddr`
  dukglue_register_property(ctx, &MantisRequest::getLocalAddr, nullptr,
                            "localAddr");
  // `req.localPort`
  dukglue_register_property(ctx, &MantisRequest::getLocalPort, nullptr,
                            "localPort");

  // `req.hasKey("key")` -> return true if key is in the context store
  dukglue_register_method(ctx, &MantisRequest::hasKey, "hasKey");
  // `req.set("key", value)` // Store in the context store the given key and
  // value. Note only int, float, double, str
