...
namespace mantis {
    ...
    std::optional<std::string> Validators::requiredConstraintCheck(const json &field, const json &body) {
        ...
        if (required && body[field_name].is_null()) {
    ...
    std::optional<std::string> Validators::viewTypeSQLCheck(const json &body) {
        if (!body.contains("sql") || body["sql"].is_null() || trim(body["sql"].get<std::string>()).empty()) {
    ...
    std::optional<std::string> Validators::validateUpdateRequestBody(const Entity &schema, const json &body) {
        return std::nullopt;
    }

    // std::optional<std::string> Validators::validateUpdateRequestBody(const json &body) {
    //     // If the table type is of view type, check that the SQL is passed in ...
    //     if (m_tableType == "view") {
    //         const auto err = viewTypeSQLCheck(body);
    ...
    //             {
    //                 // REQUIRED CONSTRAINT CHECK
    //                 const auto err = requiredConstraintCheck(field, body);
    //                 if (err.has_value()) return err;
    //             }
    //
    //             {
    //                 // MINIMUM CONSTRAINT CHECK
    //                 const auto err = minimumConstraintCheck(field, body);
    //                 if (err.has_value()) return err;
    //             }
    //
    //             {
    //                 // MAXIMUM CONSTRAINT CHECK
    //                 const auto err = maximumConstraintCheck(field, body);
    //                 if (err.has_value()) return err;
    //             }
    //
    //             {
    //                 // VALIDATOR CONSTRAINT CHECK
    //                 const auto err = validatorConstraintCheck(field, body);
    //                 if (err.has_value()) return err;
    //             }
    //         }
    //     }
