...
docker build -t mantisbase -f docker/Dockerfile --build-arg MB_VERSION=0.3.3 .
...
This builds the image (using `MB_VERSION` build arg, default `0.3.3`) and starts the container with volume mounts configured. Set `MB_JWT_SECRET` and other `MB_*` variables in a `.env` file in the same directory or pass them when running `docker compose`.
...
| `MB_VERSION` | Build-time only: release version to download (Dockerfile `ARG`) | `0.3.3` |
